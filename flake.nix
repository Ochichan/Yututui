      ...
      packages = forAllSystems (system: pkgs:
        let
          ...
          yututui = pkgs.rustPlatform.buildRustPackage {
            pname = "yututui";
            version = "1.6.33"; # keep in sync with Cargo.toml
          ...
          guiDist = pkgs.buildNpmPackage {
            pname = "yututui-gui";
            version = "1.6.33"; # private GUI package version; not part of the release surface
            src = ./gui;
          ...
          yututui-desktop = pkgs.rustPlatform.buildRustPackage {
            ...
            version = "1.6.33"; # keep the yututray binary version in sync with Cargo.toml
            src = desktopSrc;
            cargoLock.lockFile = ./Cargo.lock;
            nativeBuildInputs = [ pkgs.makeWrapper pkgs.pkg-config ];
            buildInputs = lib.optionals pkgs.stdenv.isLinux [ pkgs.openssl ];
