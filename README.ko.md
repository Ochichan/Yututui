...
> **yt-dlp는 스스로 최신을 유지합니다.** YouTube는 매주 바뀌기 때문에 배포판에 동결된 yt-dlp(예: 데비안 stable)는 결국 재생이 깨집니다. 그래서 `ytt`는 공식 스탠드얼론 빌드(nightly 채널, github.com에서 SHA-256 검증)를 `~/.local/share/ytm-tui/tools/`에 직접 받아 관리하고, {관리형, 시스템} 중 더 최신 쪽을 사용합니다. `ytt tools status`로 확인, `ytt tools update`로 즉시 갱신, `config.json`에 `"tools": {"ytdlp_channel": "stable"}`로 채널 변경, `"tools": {"ytdlp_managed": false}`로 완전 비활성화(이후 yt-dlp 최신 유지는 직접 해야 합니다). 다운로드는 무결성 검증만 되고 출처 서명은 아니므로, 앱의 실행파일 다운로드가 부담스러우면 끄세요.
...
| 어제는 됐는데 오늘은 안 됨 | YouTube가 뭔가 바꿨어요. `ytt`가 자체 yt-dlp를 자동 업데이트하고 실패한 곡을 한 번 재시도합니다. 그래도 안 되면 `ytt tools update`를 실행하거나, 시스템 yt-dlp를 업데이트하세요(`brew upgrade yt-dlp`, `scoop update yt-dlp`). |
| "스트림 해석 실패 (yt-dlp가 오래됐을 수 있음)" | 위와 같은 원인 — `ytt tools update` 후 `ytt tools status`로 어떤 yt-dlp가 활성인지 확인하세요. |
