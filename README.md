...
`yututray` and `yututray --background` start tray-only; `--mini` opens the mini player and
`--main-window` opens the full desktop window. Launching the bare command again asks the existing
instance to show its mini player instead of creating a second tray icon. On Windows, left-click
...
stay out of the taskbar/Dock and app switcher; the full main window uses normal application-window
behavior.
