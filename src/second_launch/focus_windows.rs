...
fn resolve_target(
    ...
    if let Some(recorded) = console_hwnd {
        ...
        // SAFETY: these calls only inspect the handle table/window metadata; a stale or
        // reused handle yields FALSE / pid 0 rather than UB. Visibility rejects Windows
        // Terminal's hidden pseudo-console HWND so the host-pid fallback can find its real UI.
        let live = unsafe {
