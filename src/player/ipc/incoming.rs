...
fn parse_chapter_list(value: &serde_json::Value) -> Vec<Chapter> {
...
fn dispatch_incoming(line: &str, emit: &EventSink, state: &mut DispatchState) {
    ...
    match incoming {
        MpvIncoming::PropertyChange { name, value, .. } => match name.as_str() {
            ...
            "chapter-list" => {
                emit_file_event(emit, state, PlayerEvent::Chapters(parse_chapter_list(&value)));
            }
...
mod chapter_tests {
    ...
    fn parses_ordered_chapters_and_drops_garbage() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                Chapter { title: "Drop".into(), start_secs: 120.5 },
                Chapter { title: "End".into(), start_secs: 600.0 },
            ]
        );
    }
    ...
    fn caps_and_dedups_noise_boundaries() {
        ...
        assert!(chapters.windows(2).all(|w| w[0].start_secs < w[1].start_secs));
    }
