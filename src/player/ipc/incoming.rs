...
fn reset_file_state_preserving_resume(
...
fn begin_resume_seek_observation(
    state: &mut DispatchState,
    file_generation: u64,
    request_id: u64,
) {
...
fn complete_resume_telemetry(
    ...
    let buffered = {
        let Some(gate) = state
            ...
            .as_mut()
            .filter(|gate| {
                gate.file_generation == file_generation
                    && gate.expected_request_id == Some(request_id)
            })
        else {
            return;
    ...
    match incoming {
        ...
        MpvIncoming::EndFile {
            ...
            if state
                .pending_resume
                .as_ref()
                .is_some_and(|pending| generation.is_none_or(|value| value == pending.file_generation))
            {
        ...
        MpvIncoming::CommandReply {
            ...
            if !accepted
                && let Some(contract) = pending.terminal_contract
            {
