...
async fn take_accepted_reply(replies: &Arc<Mutex<Vec<RemoteReply>>>) -> RemoteReply {
...
async fn owner_signal_barrier_flushes_a_delayed_one_shot_reply_before_hub_abort() {
    ...
    assert!(
        reply.send(RemoteResponse::err("shutting_down")),
        "accepted one-shot request still owns its response receiver"
    );
...
async fn owner_signal_barrier_flushes_a_delayed_session_reply_before_goodbye() {
    ...
    assert!(
        reply.send(RemoteResponse::err("shutting_down")),
        "accepted session request still owns its response receiver"
    );
    await_structural_barrier(&hub, 1).await;
    hub.shutdown_all();
...
async fn joined_retry_keeps_its_own_wire_token_after_the_first_peer_disconnects() {
    ...
    hub.quiesce_owner_admission();
    assert!(
        owner_reply.send(RemoteResponse::err("shutting_down")),
        "the deduped owner execution remains live"
    );
