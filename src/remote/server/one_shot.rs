...
pub(super) async fn build_tracked_response(
    ...
    if let Some(request_id) = req.request_id {
        ...
        let outcome = hub
            ...
            .execute_with_replay_proof(request_key, &req.command, reply_timeout, |reply| {
                hub.run_if_owner_admission_open(|| {
                    emit(RemoteEvent::Command(owner_command, reply.into()))
                })
                .unwrap_or(false)
            })
            .await;
        ...
        return TrackedRemoteResponse {
            response: response_for_one_shot_version(request_version, response),
            retained_replay,
    ...
    if !hub
        .run_if_owner_admission_open(|| emit(RemoteEvent::Command(req.command, reply_tx.into())))
        .unwrap_or(false)
    {
