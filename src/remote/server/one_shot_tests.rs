...
async fn one_shot_marks_only_a_retained_same_id_outcome_as_replayed() {
    ...
    tokio::spawn(serve(
        ...
        Arc::new(move |event| {
            ...
            reply.send(RemoteResponse::ok("applied".to_owned())).is_ok()
        }),
        test_hub(),
    ));
...
async fn retiring_guard_cannot_unlink_a_fast_successor() {
    let socket = std::env::temp_dir()
        .join(format!(
            "yututui-remote-successor-test-{}.sock",
            std::process::id()
        ))
        .to_string_lossy()
        .into_owned();
    let descriptor = PathBuf::from(format!("{socket}.json"));
...
async fn late_guard_cleanup_preserves_a_rebound_successor_inode() {
    let socket = std::env::temp_dir()
        .join(format!(
            "yututui-remote-rebound-test-{}.sock",
            std::process::id()
        ))
        .to_string_lossy()
        .into_owned();
...
async fn repeated_accept_failures_self_revoke_the_published_endpoint() {
    let socket = std::env::temp_dir()
        .join(format!(
            "yututui-remote-accept-failure-test-{}.sock",
            std::process::id()
        ))
        .to_string_lossy()
        .into_owned();
...
async fn failed_accept_owner_cannot_overwrite_or_unlink_a_fast_successor() {
    let socket = std::env::temp_dir()
        .join(format!(
            "yututui-remote-accept-successor-test-{}.sock",
            std::process::id()
        ))
        .to_string_lossy()
        .into_owned();
