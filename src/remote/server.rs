...
mod session_socket_tests {
    ...
    pub(super) fn start_stalled_command_server(
        ...
        String,
        Arc<std::sync::Mutex<Vec<tokio::sync::oneshot::Sender<RemoteResponse>>>>,
    ) {
        let ep = test_endpoint(tag);
        let listener = bind(&ep).unwrap();
        let held_replies = Arc::new(std::sync::Mutex::new(Vec::<
            tokio::sync::oneshot::Sender<RemoteResponse>,
        >::new()));
    ...
    async fn timed_out_session_retry_reuses_late_completion_without_duplicate_owner_mutation() {
        ...
        assert!(
            late_quick
                .send(RemoteResponse::ok("late quick".to_owned()))
                .is_ok(),
        ...
        assert!(
            late_playback
                .send(RemoteResponse::ok("late playback".to_owned()))
                .is_ok(),
    ...
    async fn shutdown_cancels_pending_command_without_waiting_for_reply_timeout() {
        ...
        assert!(
            late_reply
                .send(RemoteResponse::ok("late".to_string()))
                .is_ok(),
