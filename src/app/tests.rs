...
#[test]
fn playing_under_a_filter_queues_only_the_filtered_subset() {
    // WYSIWYG: with a filter applied, Enter plays — and queues — only the visible matches,
    // consistent with delete/favorite/download all operating on the filtered set.
    let mut app = app_with_favorites(vec![
        fsong("a", "Lovely", "Billie Eilish"),
        fsong("b", "Bad Guy", "Billie Eilish"),
        fsong("c", "Anti-Hero", "Taylor Swift"),
    ...
    app.update(Msg::Key(key(KeyCode::Enter))); // play from the filtered view
    assert_eq!(app.mode, Mode::Player);
