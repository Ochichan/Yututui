...
fn ime_scrub_requires_a_matching_successfully_rendered_import_projection() {
    ...
    assert_eq!(
        app.ime_scrub_local_projection_fresh(),
        EXISTING_DIRECTORY_GENERATION_RELIABLE,
        "non-Windows import directories must fail closed without a trustworthy generation"
    );
...
fn cold_large_unrecognized_file_has_zero_content_or_artifact_io_on_warm_lookup() {
    ...
    assert_eq!(first.1, EXISTING_DIRECTORY_GENERATION_RELIABLE);
...
fn recognized_cap_overflow_and_shrink_preserve_the_bounded_cache_contract() {
    ...
    assert_eq!(at_cap.1, EXISTING_DIRECTORY_GENERATION_RELIABLE);
    ...
    assert_eq!(recovered.1, EXISTING_DIRECTORY_GENERATION_RELIABLE);
...
fn unrelated_membership_churn_rescans_but_keeps_the_semantic_row_cache_key() {
    ...
    assert_eq!(
        std::rc::Rc::ptr_eq(&before.data, &after_add.data),
        EXISTING_DIRECTORY_GENERATION_RELIABLE
    );
    let add_probe = app
        .local_mode
    ...
    assert_eq!(
        std::rc::Rc::ptr_eq(&before.data, &after_remove.data),
        EXISTING_DIRECTORY_GENERATION_RELIABLE
    );
}
...
fn read_dir_error_is_volatile_and_recovery_redetects_membership() {
    ...
    assert_eq!(
        app.local_mode.rows_cache.borrow().is_some(),
        EXISTING_DIRECTORY_GENERATION_RELIABLE
    );
    let cached = app.local_rows_snapshot();
    assert_eq!(
        std::rc::Rc::ptr_eq(&recovered.data, &cached.data),
        EXISTING_DIRECTORY_GENERATION_RELIABLE
    );
}
...
fn stable_import_snapshot_reuses_and_memoizes_its_action_hint() {
    ...
    let cached = app.local_rows_snapshot();
    assert_eq!(
        std::rc::Rc::ptr_eq(&snapshot.data, &cached.data),
        EXISTING_DIRECTORY_GENERATION_RELIABLE
    );
