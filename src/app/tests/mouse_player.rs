...
fn drag_on_seekbar_scrubs_continuously() {
    ...
    let cmds = app.update(Msg::MouseClick { col: 25, row: 5 });
    match cmds.as_slice() {
        [cmd] => match cmd.player_command() {
...
fn rejected_seekbar_drag_can_retry_the_same_cell() {
    ...
    let first = app.update(Msg::MouseClick { col: 25, row: 5 });
    assert!(matches!(
        first.as_slice(),
...
fn unrelated_seek_rejection_does_not_arm_a_mouse_retry() {
    ...
    let press = app.update(Msg::MouseClick { col: 25, row: 5 });
...
fn click_next_button_loads_next_track() {
    ...
    let mut cmds = app.update(Msg::MouseClick { col: 3, row: 1 });
