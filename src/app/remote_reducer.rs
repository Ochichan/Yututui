...
impl App {
    ...
    fn remote_queue_play(&mut self, position: usize) -> (RemoteResponse, Vec<Cmd>) {
        ...
        // Parity with the daemon owner: a remote queue mutation may leave the queue low,
        // so top up streaming (guarded/idempotent) rather than gapping.
    ...
    fn remote_set_streaming(&mut self, state: ToggleState) -> (RemoteResponse, Vec<Cmd>) {
        ...
        // Never silently change the requested mode. A correlated desktop/API caller needs a
        // stable error code it can explain and surface as a toast.
...
mod tests {
    ...
    fn remote_cycle_repeat_refused_while_streaming_on() {
        ...
        let (response, commands) = app.apply_remote(RemoteCommand::CycleRepeat);
        assert!(!response.ok);
        assert_eq!(
