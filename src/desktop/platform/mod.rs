...
pub(crate) fn next_webview_generation() -> u64 {
    ...
    // Process-global across panel host lifetimes. A destroyed gen-1 window must
    // never alias a newly-created gen-1 window and accept its late correlated reply.
    static NEXT: AtomicU64 = AtomicU64::new(1);
    NEXT.fetch_add(1, Ordering::Relaxed)
}

/// One explicit WebView2 user-data folder for the `yututray` mini-player webview:
/// the data lands in a known, purgeable spot instead of the exe-adjacent default
/// (which breaks under a read-only install dir). The context only feeds
/// `CreateCoreWebView2Environment` at build time, so each build may hold its own
