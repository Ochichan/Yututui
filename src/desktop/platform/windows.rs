        ...
        match ev {
            ...
            gateway::GatewayEvent::Push {
                ...
                if self.desktop_app.apply_push(sequence, topic, event) {
                    // Apply the shared sequence gate before any consumer sees the
                    // frame.  Previously the main WebView received stale pushes even
                    // though the native projection correctly discarded them.
                    if let Some(main) = &self.main_window {
