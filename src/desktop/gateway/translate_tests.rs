...
/// Pin the queue-order frontend names (with the exact payload their stores emit) to the
/// enum variants, so a rename on either side fails here instead of as a runtime bad_command.
#[test]
fn queue_order_frontend_names_map_to_remote_commands() {
    let cases: Vec<(&str, serde_json::Value, RemoteCommand)> = vec![
        ...
        // The keyboard path sends no revision; the guard is optional.
        (
