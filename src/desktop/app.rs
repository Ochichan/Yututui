...
impl DesktopApp {
    ...
    pub fn apply_push(&mut self, sequence: u64, topic: Topic, event: PushEvent) -> bool {
        ...
        let applied = match (topic, event) {
            ...
            // Other GUI-only topics still advance the global sequence. They are valid pushes,
            // just not part of the compact desktop projection.
            (Topic::Lyrics, _)
            | (Topic::Artwork, _)
            | (Topic::Library, _)
            | (Topic::Playlists, _)
            | (Topic::Search, PushEvent::SearchCompleted { .. })
            | (Topic::Ai, _)
            | (Topic::Downloads, _)
            | (Topic::Transfer, _)
            | (Topic::Accounts, _) => true,
            _ => false,
        };
        if applied {
