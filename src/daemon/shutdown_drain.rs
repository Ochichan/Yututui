...
pub(super) async fn drain_daemon_shutdown_ingress(
    ...
    while let Some(event) = pending_events.pop_front() {
        settle_event(event_tx, publisher, event, &mut drain);
    }
    ...
    for event in event_tx.drain_coalesced() {
        ...
        settle_non_wake(publisher, event, &mut drain);
    }
    event_rx.close();
    drain
}
...
fn settle_event(
    ...
    if matches!(event, DaemonEvent::TelemetryWake) {
        for event in event_tx.drain_coalesced() {
            drain.coalesced_events += 1;
            settle_non_wake(publisher, event, drain);
        }
    } else {
        settle_non_wake(publisher, event, drain);
    }
...
fn settle_non_wake(publisher: &Publisher, event: DaemonEvent, drain: &mut DaemonShutdownDrain) {
    let kind = event.kind();
