...
pub(crate) fn parse_csi(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    ...
    let input_event = match buffer[2] {
        ...
        b';' => {
            // A CSI sequence that starts with an omitted parameter is incomplete until its
            // final byte arrives. Parsing `ESC [` + `;` immediately used to clear the Unix
            // input buffer before the rest of a Win32-input record could arrive.
            let last_byte = buffer[buffer.len() - 1];
            if !(64..=126).contains(&last_byte) {
                None
            } else if last_byte == b'_' {
                return parse_csi_win32_input(buffer);
            } else {
                return parse_csi_modifier_key_code(buffer);
            }
        }
        b'_' => return parse_csi_win32_input(buffer),
...
// Windows KEY_EVENT_RECORD control-state flags. They are duplicated here instead of importing
// winapi because this parser is built on Unix, where crossterm's Windows-only dependencies are not
// available.
...
fn win32_input_modifiers(control_state: u32) -> KeyModifiers {
    let mut modifiers = KeyModifiers::empty();
    if control_state & WIN32_SHIFT_PRESSED != 0 {
        modifiers |= KeyModifiers::SHIFT;
    }
    if control_state & (WIN32_LEFT_ALT_PRESSED | WIN32_RIGHT_ALT_PRESSED) != 0 {
        modifiers |= KeyModifiers::ALT;
    }
    if control_state & (WIN32_LEFT_CTRL_PRESSED | WIN32_RIGHT_CTRL_PRESSED) != 0 {
        modifiers |= KeyModifiers::CONTROL;
    }
    modifiers
}

fn win32_modifier_key(
    virtual_key_code: u16,
    virtual_scan_code: u16,
    control_state: u32,
) -> Option<ModifierKeyCode> {
    match virtual_key_code {
        0x10 => Some(if virtual_scan_code == 0x36 {
            ModifierKeyCode::RightShift
        } else {
            ModifierKeyCode::LeftShift
        }),
        0x11 => Some(
            if control_state & (WIN32_RIGHT_CTRL_PRESSED | WIN32_ENHANCED_KEY) != 0 {
        ...
        0x12 => Some(
            if control_state & (WIN32_RIGHT_ALT_PRESSED | WIN32_ENHANCED_KEY) != 0 {
                ModifierKeyCode::RightAlt
            } else {
                ModifierKeyCode::LeftAlt
            },
        ),
        0x5B => Some(ModifierKeyCode::LeftSuper),
        0x5C => Some(ModifierKeyCode::RightSuper),
        0xA0 => Some(ModifierKeyCode::LeftShift),
        0xA1 => Some(ModifierKeyCode::RightShift),
        0xA2 => Some(ModifierKeyCode::LeftControl),
        0xA3 => Some(ModifierKeyCode::RightControl),
        0xA4 => Some(ModifierKeyCode::LeftAlt),
        0xA5 => Some(ModifierKeyCode::RightAlt),
        _ => None,
    }
}

fn win32_fallback_character(virtual_key_code: u16) -> Option<char> {
    match virtual_key_code {
        0x20 => Some(' '),
        0x30..=0x39 => char::from_u32(u32::from(virtual_key_code)),
        0x41..=0x5A => char::from_u32(u32::from(virtual_key_code) + u32::from(b'a' - b'A')),
        0x60..=0x69 => char::from_u32(u32::from(virtual_key_code - 0x60) + u32::from(b'0')),
        0x6A => Some('*'),
        0x6B => Some('+'),
        0x6C => Some(','),
        0x6D => Some('-'),
...
fn win32_character_key_code(
    virtual_key_code: u16,
    unicode_char: u16,
    modifiers: KeyModifiers,
) -> io::Result<KeyCode> {
    // Printable Unicode comes from the user's active layout and must win over the US-layout
    // virtual key. Control combinations instead carry a C0 code, so use the VK to retain the
    // actual identity (`VK_H + U+0008` is Ctrl+H, not Backspace).
    if unicode_char > 0x1F && unicode_char != 0x7F {
        return char::from_u32(u32::from(unicode_char))
            .map(KeyCode::Char)
            .ok_or_else(could_not_parse_event_error);
    }

    if let Some(ch) = win32_fallback_character(virtual_key_code) {
        return Ok(KeyCode::Char(ch));
    }

    match unicode_char {
        0x09 if modifiers.contains(KeyModifiers::SHIFT) => Ok(KeyCode::BackTab),
        0x09 => Ok(KeyCode::Tab),
        0x0D => Ok(KeyCode::Enter),
        0x1B => Ok(KeyCode::Esc),
        0x7F => Ok(KeyCode::Backspace),
        0x01..=0x1A if modifiers.contains(KeyModifiers::CONTROL) => {
            let offset =
                u8::try_from(unicode_char - 1).map_err(|_| could_not_parse_event_error())?;
            Ok(KeyCode::Char(char::from(b'a' + offset)))
        }
        _ => Err(could_not_parse_event_error()),
    }
}
...
fn win32_key_code(
    ...
    if let Some(modifier) = win32_modifier_key(virtual_key_code, virtual_scan_code, control_state) {
        return Ok(KeyCode::Modifier(modifier));
    }

    let code = match virtual_key_code {
        0x08 => KeyCode::Backspace,
        0x09 if modifiers.contains(KeyModifiers::SHIFT) => KeyCode::BackTab,
        0x09 => KeyCode::Tab,
        0x0C => KeyCode::KeypadBegin,
        0x0D => KeyCode::Enter,
        0x13 => KeyCode::Pause,
        ...
        0x27 => KeyCode::Right,
        0x28 => KeyCode::Down,
        0x2A | 0x2C => KeyCode::PrintScreen,
        0x2D => KeyCode::Insert,
        0x2E => KeyCode::Delete,
        0x5D => KeyCode::Menu,
        0x70..=0x87 => KeyCode::F(u8::try_from(virtual_key_code - 0x6F).unwrap_or(24)),
        0x90 => KeyCode::NumLock,
        0x91 => KeyCode::ScrollLock,
        0xAD => KeyCode::Media(MediaKeyCode::MuteVolume),
        0xAE => KeyCode::Media(MediaKeyCode::LowerVolume),
        0xAF => KeyCode::Media(MediaKeyCode::RaiseVolume),
        0xB0 => KeyCode::Media(MediaKeyCode::TrackNext),
        0xB1 => KeyCode::Media(MediaKeyCode::TrackPrevious),
        0xB2 => KeyCode::Media(MediaKeyCode::Stop),
        0xB3 => KeyCode::Media(MediaKeyCode::PlayPause),
        0xFA => KeyCode::Media(MediaKeyCode::Play),
        _ => return win32_character_key_code(virtual_key_code, unicode_char, modifiers),
    };
    Ok(code)
}

fn win32_keypad_state(virtual_key_code: u16, control_state: u32) -> KeyEventState {
    let explicitly_keypad = matches!(virtual_key_code, 0x0C | 0x60..=0x6F)
        || (virtual_key_code == 0x0D && control_state & WIN32_ENHANCED_KEY != 0);
    let keypad_navigation = matches!(virtual_key_code, 0x21..=0x28 | 0x2D | 0x2E)
        && control_state & WIN32_ENHANCED_KEY == 0;

    if explicitly_keypad || keypad_navigation {
...
fn add_modifier_key_state(code: KeyCode, modifiers: &mut KeyModifiers) {
...
/// Parse Windows Terminal's `win32-input-mode` representation of a KEY_EVENT_RECORD.
///
/// yututui patch: Konsole 26.04 and Windows Terminal can emit this protocol on Unix PTYs,
/// allowing callers to distinguish physical Ctrl+Backspace from Ctrl+H.
pub(crate) fn parse_csi_win32_input(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    assert!(buffer.starts_with(b"\x1B["));
    assert!(buffer.ends_with(b"_"));

    let parameters = std::str::from_utf8(&buffer[2..buffer.len() - 1])
        .map_err(|_| could_not_parse_event_error())?;
    let mut split = parameters.split(';');

    let virtual_key_code = next_win32_parameter::<u16>(&mut split, 0)?;
    let virtual_scan_code = next_win32_parameter::<u16>(&mut split, 0)?;
    let unicode_char = next_win32_parameter::<u16>(&mut split, 0)?;
    let key_down = next_win32_parameter::<u8>(&mut split, 0)?;
    let control_state = next_win32_parameter::<u32>(&mut split, 0)?;
    let repeat_count = next_win32_parameter::<u16>(&mut split, 1)?;

    if split.next().is_some() || key_down > 1 {
        return Err(could_not_parse_event_error());
    }

    let mut modifiers = win32_input_modifiers(control_state);
    let code = win32_key_code(
        virtual_key_code,
        virtual_scan_code,
        unicode_char,
        modifiers,
    ...
    add_modifier_key_state(code, &mut modifiers);

    let kind = if key_down == 0 {
    ...
    let mut state = win32_keypad_state(virtual_key_code, control_state);
    if control_state & WIN32_CAPSLOCK_ON != 0 {
        state |= KeyEventState::CAPS_LOCK;
    }
    if control_state & WIN32_NUMLOCK_ON != 0 {
...
mod tests {
    ...
    #[test]
    fn test_parse_csi_win32_input_distinguishes_ctrl_backspace_and_ctrl_h() {
        assert_eq!(
            parse_csi(b"\x1B[8;0;8;1;8;1_").unwrap(),
            win32_key(
                KeyCode::Backspace,
                KeyModifiers::CONTROL,
                KeyEventKind::Press,
        ...
        assert_eq!(
            parse_csi(b"\x1B[72;0;8;1;8;1_").unwrap(),
            win32_key(
                KeyCode::Char('h'),
                KeyModifiers::CONTROL,
                KeyEventKind::Press,
    ...
    #[test]
    fn test_parse_csi_win32_input_defaults_and_event_kinds() {
        assert_eq!(
            parse_csi_win32_input(b"\x1B[65;30;97;1_").unwrap(),
            win32_key(
                KeyCode::Char('a'),
                KeyModifiers::NONE,
                KeyEventKind::Press,
        ...
        assert_eq!(
            parse_csi_win32_input(b"\x1B[65;30;97_").unwrap(),
            win32_key(
                KeyCode::Char('a'),
                KeyModifiers::NONE,
                KeyEventKind::Release,
        ...
        assert_eq!(
            parse_csi_win32_input(b"\x1B[65;30;97;1;;3_").unwrap(),
            win32_key(
                KeyCode::Char('a'),
                KeyModifiers::NONE,
                KeyEventKind::Repeat,
        ...
        assert_eq!(
            parse_csi_win32_input(b"\x1B[112;59;;1;8_").unwrap(),
            win32_key(
                KeyCode::F(1),
                KeyModifiers::CONTROL,
                KeyEventKind::Press,
    ...
    #[test]
    fn test_parse_csi_win32_input_waits_for_fragmented_record() {
        let sequence = b"\x1B[;;97;1_";
        for end in 1..sequence.len() {
            assert_eq!(parse_event(&sequence[..end], true).unwrap(), None);
        }
        assert_eq!(
    ...
    #[test]
    fn test_parse_csi_win32_input_modifiers_and_lock_state() {
        assert_eq!(
            parse_csi_win32_input(b"\x1B[17;29;0;1;260;1_").unwrap(),
            win32_key(
        ...
        assert_eq!(
            parse_csi_win32_input(b"\x1B[17;29;0;0;256;1_").unwrap(),
            win32_key(
                KeyCode::Modifier(ModifierKeyCode::RightControl),
                KeyModifiers::CONTROL,
                KeyEventKind::Release,
        ...
        assert_eq!(
            parse_csi_win32_input(b"\x1B[16;54;0;1;16;1_").unwrap(),
            win32_key(
        ...
        assert_eq!(
            parse_csi_win32_input(b"\x1B[65;30;65;1;176;1_").unwrap(),
            win32_key(
                KeyCode::Char('A'),
                KeyModifiers::SHIFT,
                KeyEventKind::Press,
    ...
    #[test]
    fn test_parse_csi_win32_input_unicode_function_and_keypad_keys() {
        assert_eq!(
            parse_csi_win32_input(b"\x1B[72;0;54620;1;0;1_").unwrap(),
            win32_key(
                KeyCode::Char('한'),
                KeyModifiers::NONE,
                KeyEventKind::Press,
        ...
        assert_eq!(
            parse_csi_win32_input(b"\x1B[38;0;0;1;0;1_").unwrap(),
            win32_key(
                KeyCode::Up,
                KeyModifiers::NONE,
                KeyEventKind::Press,
        ...
        assert_eq!(
            parse_csi_win32_input(b"\x1B[38;0;0;1;256;1_").unwrap(),
            win32_key(
                KeyCode::Up,
                KeyModifiers::NONE,
                KeyEventKind::Press,
        ...
        assert_eq!(
            parse_csi_win32_input(b"\x1B[13;0;13;1;256;1_").unwrap(),
            win32_key(
                KeyCode::Enter,
                KeyModifiers::NONE,
                KeyEventKind::Press,
        ...
        assert_eq!(
            parse_csi_win32_input(b"\x1B[187;0;0;1;8;1_").unwrap(),
            win32_key(
                KeyCode::Char('='),
                KeyModifiers::CONTROL,
                KeyEventKind::Press,
    ...
    #[test]
    fn test_parse_csi_win32_input_rejects_malformed_records() {
        for malformed in [
            b"\x1B[65;30;97;2;0;1_".as_slice(),
            b"\x1B[65;30;97;1;0;1;7_".as_slice(),
            b"\x1B[65536;30;97;1;0;1_".as_slice(),
            b"\x1B[65;30;97;1;4294967296;1_".as_slice(),
            b"\x1B[65;nope;97;1;0;1_".as_slice(),
            b"\x1B[65;30;55296;1;0;1_".as_slice(),
            b"\x1B[_".as_slice(),
        ] {
            assert!(parse_csi_win32_input(malformed).is_err(), "{malformed:?}");
        }
    }
