...
Assert-File "ytt-tray-version.txt" | Out-Null
...
Assert-True ($results.ytt_tray_subsystem -eq 2) "ytt-tray.exe was not recorded as a GUI subsystem binary"
...
Assert-True (-not $trayBefore.Contains("Id")) "ytt-tray.exe was already running before manual QA"
...
Assert-True ($startupRegistry.Contains("ytt-tray.exe")) "startup registry value did not include ytt-tray.exe"
...
Assert-True ($trayAfterQuitPlayer.Contains("Id")) "ytt-tray.exe was not alive after Quit Player"

$trayAfterQuitTray = Read-TextEvidence "tray-process-after-quit-tray.txt"
Assert-True (-not $trayAfterQuitTray.Contains("Id")) "ytt-tray.exe was still running after Quit Tray"
