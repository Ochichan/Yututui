...
**Tray 補助アプリ:** macOS と Windows のリリースには、メニューバー / 通知領域のミニプレイヤー `ytt-desktop` が含まれます。
...
以上。音楽が流れます。（何かおかしい？ **`ytt doctor`** が直すべき箇所を正確に教えてくれます。）

## ツアー

スクリーンショットと GIF は近日ここに追加されます — それまでは **[機能ツアー](https://ochichan.github.io/ytm-tui/)** ですべてをライブで、詳しく見られます。
...
**`v`** で小さな mpv ウィンドウに MV が浮かびます。*動画の自動連続再生*をオンにすると、次の曲の MV へ自動で続きます。
...
### カタログは六つ、検索窓は一つ — さらにラジオモード

検索で **`Tab`** を押すと YouTube Music、SoundCloud、Audius、Jamendo、Internet Archive、Radio Browser を行き来できます（全部まとめても可）。**`Alt+Shift+R`** はアプリ全体をネットラジオのチューナーに変えます。

> 🖼️ *スクリーンショットは近日追加予定。*
<!-- 📸 埋め方: docs/media/sources.png を追加し、上の「近日追加」行を消して、下の2つのコメントを外してください:
![一つの検索窓から六つのカタログを検索](docs/media/sources.png)
-->
<!-- 📸 埋め方: docs/media/radio.png を追加してコメントを外す:
...
テーマ13種（34の色ロールすべて hex で編集可能）、アニメーション25種 — くるくる回る ASCII ドーナツまで含めて。

> 🖼️ *GIF は近日追加予定。*
<!-- 📸 埋め方: docs/media/themes.gif を追加し、上の「近日追加」行を消して、下の2つのコメントを外してください:
...
`ytt transfer import <url>` — チェックポイント、再開、あいまいな曲はマッチレポートへ。設定方法は下のリファレンスに。
...
- **ダウンロード** — `d` でカバーアートとタグ入りの m4a に保存、オフライン再生できます。
- **スクロブル** — Last.fm / ListenBrainz。クラッシュに強いオフラインキュー付き。
- **10バンド EQ** のプリセット + ラウドネスノーマライズ。
- 全キー再設定可能、UI 全体がマウス対応、インターフェースは English & 한국어。

**すべての詳しい話 → [機能ツアー](https://ochichan.github.io/ytm-tui/)。**
...
> **ハングル配列でも大丈夫。** ショートカットは 2ボル式の字母を理解します（`ㅂ` は `q` として効く）— IME を切り替える必要はありません。マウス派なら画面のすべてがクリックでき、ホイールは音量に効きます。
...
**初回のみの設定。** Development Mode の Spotify アプリは自分の許可リストにあるアカウントしか受け付けないので、自分の（無料の）アプリを作ります: [developer.spotify.com/dashboard](https://developer.spotify.com/dashboard) でアプリを作成し、リダイレクト URI に `http://127.0.0.1:9271/callback` を**正確に**追加し（ループバック IP、`localhost` ではない; ポートは `spotify.redirect_port`）、*User Management* に自分のアカウントを追加して、Client ID を設定 → アカウントに貼ります。クライアントシークレットはありません — PKCE は使わないので。

マッチングはメタデータベースです（NFKC 正規化、CJK 安全）。あいまいな曲は黙って当てずっぽうにせず、ジョブレポートに残ります — `--take-best` / `--min-score` で再実行するか、大きなプレイリストは `--dry-run` で確認してから `ytt transfer resume <job-id>` で書き込みを。
...
| Spotify で 403 | Spotify 開発者ダッシュボードの *User Management* に自分のアカウントを追加。 |
