...
fn wait_for_controller_ready(
    ...
    loop {
        ...
        if let Ok(bytes) = std::fs::read(path)
            ...
            && controller_ready_matches_exact_identity(&value, root_pid, owner, run_id, last_mpv)
        {
            return Ok(());
        }
        if let Some(status) = child
...
fn controller_ready_matches_exact_identity(
    value: &serde_json::Value,
    root_pid: u32,
    owner: &LiveProcessIdentity,
    run_id: &str,
    live_mpv: &[MpvIdentity],
) -> bool {
    if value.get("schema").and_then(serde_json::Value::as_str)
        != Some("ytt.tui-perf.controller-ready.v1")
        || value.get("owner_pid").and_then(serde_json::Value::as_u64) != Some(u64::from(root_pid))
        || value
            .get("owner_start_time_unix_s")
            .and_then(serde_json::Value::as_u64)
            != Some(owner.start_time_unix_s)
    ...
    let Some(mpv_start) = value
        .get("mpv_start_time_unix_s")
        .and_then(serde_json::Value::as_u64)
    else {
        return false;
    };
    let Some(mpv_endpoint) = value
        .get("mpv_endpoint")
        .and_then(serde_json::Value::as_str)
    else {
        return false;
    };
    let endpoint_argv = format!("--input-ipc-server={mpv_endpoint}");
    live_mpv.iter().any(|identity| {
        identity.pid == mpv_pid
            && identity.start_time_unix_s == mpv_start
            && identity
                .input_ipc_server_argv
                .iter()
                .any(|arg| arg == &endpoint_argv)
    })
}
...
mod tests {
    ...
    use super::{
        ...
        emergency_shutdown_spawned_tree, emergency_shutdown_spawned_tree_with_group_signal,
        exact_mpv_identity_matches, header_record, input_ipc_server_argv,
        live_identity_update_is_stale, producer_binary_sha256, sha256_file,
        shutdown_child_with_grace, shutdown_measured_tree,
    ...
    #[test]
    fn controller_barrier_requires_the_exact_live_owner_and_mpv_endpoint_identity() {
        let owner = LiveProcessIdentity {
            pid: 40,
            start_time_unix_s: 400,
            process_group_id: None,
            executable: PathBuf::from("/ytt"),
            executable_bytes: 1,
            executable_sha256: "owner".to_string(),
        };
        let mpv = MpvIdentity {
            pid: 41,
            start_time_unix_s: 410,
            executable: PathBuf::from("/mpv"),
            executable_bytes: 1,
            executable_sha256: "mpv".to_string(),
            input_ipc_server_argv: vec!["--input-ipc-server=/tmp/exact.sock".to_string()],
        };
        let mut ready = serde_json::json!({
            "schema": "ytt.tui-perf.controller-ready.v1",
            "run_id": "run-1",
            "owner_pid": 40,
            "owner_start_time_unix_s": 400,
            "mpv_pid": 41,
            "mpv_start_time_unix_s": 410,
            "mpv_endpoint": "/tmp/exact.sock",
            "subscriptions_confirmed": true,
        });

        assert!(controller_ready_matches_exact_identity(
            &ready,
            40,
            &owner,
            "run-1",
            std::slice::from_ref(&mpv),
        ));
        ready["mpv_endpoint"] = serde_json::json!("/tmp/stale.sock");
        assert!(!controller_ready_matches_exact_identity(
            &ready,
            40,
            &owner,
        ...
        ready["mpv_endpoint"] = serde_json::json!("/tmp/exact.sock");
        ready["owner_start_time_unix_s"] = serde_json::json!(401);
        assert!(!controller_ready_matches_exact_identity(
            &ready,
            40,
            &owner,
